...
//Array of power meter readings
float meterReadings[10];
...
{
  //Serial.println("executeCurrentRegistryFunction");
  for(int i = 0; i < ARRAY_SIZE(Registries); i++)
  {
    Registries[currentRegistryNumber]();
    nextRegistryNumber();
  }
  for(int i = 0; i < ARRAY_SIZE(meterReadings); i++)
  {
    Serial.print(meterReadings[i]);
    Serial.print(" ");
  }
  Serial.println();
}
...
void AddressRegistry_3100()
{
  ...
  //Serial.println("AddressRegistry_3100");
  ...
  if (result == node.ku8MBSuccess)
  {
    ...
    meterReadings[3] = bvoltage;


    /**
    Shahrom: 
      0. Define HEATUP_TO_WARM threshold value : 12.65Volts? 
      1. check if connected PC is on (over USB)
      2. If the PC is not on and bVoltage is above the HEATUP_TO_WARM threshold, send wake signal to PC 
    **/
    ...
    meterReadings[4] = battChargeCurrent;
...
void AddressRegistry_3106()
{
  ...
  //Serial.println("AddressRegistry_3106");
  ...
  if (result == node.ku8MBSuccess)
  {
    ...
    meterReadings[5] = battChargePower;
...
{
    // display.clear();
  //Serial.println("AddressRegistry_310D");
...
{
    // display.clear();
  //Serial.println("AddressRegistry_311A");
  result = node.readInputRegisters(0x311A, 2);

  if (result == node.ku8MBSuccess)
  {
    bremaining = node.getResponseBuffer(0x00) / 1.0f;
    meterReadings[7] = bremaining;
    //Serial.print("Battery Remaining %: ");
    //Serial.println(bremaining);

    btemp = node.getResponseBuffer(0x01) / 100.0f;
    meterReadings[8] = btemp;
...
{
    // display.clear();
  //Serial.println("AddressRegistry_331B");
  ...
  if (result == node.ku8MBSuccess)
  {
    ...
    meterReadings[9] = battOverallCurrent;
